...
class RegulationController extends Controller
{
    ...
    private $route = 'admin.help.regulation';
    private $config, $layout;
    private $title = 'Regulasi';
    private $permission = 'help-regulation';
    ...
    public function index()
    {
        ...
        $data['regulations'] = Regulation::where('id_parent', null)->orderBy('type')
            ->orderBy('order')->get();
    ...
    public function show($id = 0)
    {
        ...
        $data['config'] = $this->config;
        $data['regulations'] = Regulation::where('id_parent', $id)->orderBy('type')->get();
    ...
    public function data($id = null)
    {
        ...
        $data['id'] = $id;
        $data['config']['pageTitle'] = [route($this->route . '.data') => 'Regulasi'];
        if($id != null) {
            $reg = Regulation::with('allParent')->where('id', $id)->first();
    ...
    public function table($id = null)
    {
        ...
        $dataTable = Datatable::create($model)
            ->editColumn('filename', function ($data){
                return $data->type == 'folder' ? $data->filename : '<a href="'.asset('file/regulations/'.$data->file).'">'.$data->filename.'</a>';
            })
    ...
    public function store(Request $request, $type, $id = null)
    {
        ...
        if (!$validator->fails()) {
            ...
            if($type == 'file') {
                ...
                $file->move(public_path('file/regulations'), $param['file']);
            } else {
                $param['file'] = null;
                $param['filetype'] = $type;
            }
