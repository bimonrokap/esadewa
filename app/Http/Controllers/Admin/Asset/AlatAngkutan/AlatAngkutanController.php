...
namespace App\Http\Controllers\Admin\Asset\AlatAngkutan;
...
class AlatAngkutanController extends AssetController
{

    private $route = 'admin.asset.alatangkutanbermotor';
    private $title = 'Alat Angkutan Bermotor';
    public $tableHeader = ['Kode Barang', 'NUP', 'Kode Satker', 'Nama Satker', 'KIB', 'Nama Barang', 'Kondisi', 'Merk/Type', 'Tgl Perolehan', 'Nilai Perolehan Pertama', 'Nilai Mutasi', 'Nilai Perolehan', 'Nilai Penyusutan', 'Nilai Buku', 'Kuantitas', 'Jml Foto', 'Status Penggunaan', 'Status Pengelolaan', 'No PSP', 'Tgl PSP', 'No BPKB', 'No Polisi', 'Pemakai', 'Jml KIB', 'Tanggal Update'];
    protected $categoryAsset = 4;

    public function __construct()
    {
        $this->model = new AssetAlatBermotor();
    ...
    public function index()
    {
        ...
        $data['importSlug'] = 'bermotor';

        $data['filter'] = $this->generalFilter();
        $data['baseFilter'] = array_merge($this->baseFilter, ['kendaraan_type' => 'kendaraan_type']);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$validator->fails()) {
            ...
            $data = AssetVariable::whereVariable('pemakai')->whereIdAsset($idAsset)->first();
            if($data == null){
                AssetVariable::create([
                    'id_asset' => $idAsset,
                    'variable' => 'pemakai',
                    'value' => $request->input('pemakai'),
                    'created_by' => $user->id
                ]);
            } else {
                $data->value = $request->input('pemakai');
                $data->save();
            }
